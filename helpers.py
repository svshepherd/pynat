...
DEFAULT_INATURALISMUS_PLACES = {
    1491: 'Powhatan County',
    2920: 'Goochland County',
    3032: 'Louisa County',
}

DEFAULT_INATURALISMUS_CONFIG = {
    'places': DEFAULT_INATURALISMUS_PLACES,
    'target_loc': (37.66933, -77.81001, 42),
    'coming_soon_norm': 'overall',
    'coming_soon_limit': 7,
    'coming_soon_fetch_images': False,
    'coming_soon_kinds': ['flowers', 'fruits', 'mushrooms', 'birds', 'herps', 'mammals', 'wugs'],
    'include_lifestages': True,
    'hist_start_year': '2008-01-01',
    'hist_start_month': '2016-01-01',
    'throttle_seconds': 0.25,
}


def _display_md(text: str) -> None:
    if HAS_IPYTHON_DISPLAY and Markdown is not None:
        display(Markdown(text))
    else:
        print(text)


def _extract_place_ids(places: Union[dict[int, str], list[int], tuple[int, ...], None]) -> list[int]:
    if places is None:
        return list(DEFAULT_INATURALISMUS_PLACES.keys())
    if isinstance(places, dict):
        return [int(k) for k in places.keys()]
    return [int(p) for p in places]


def _inat_total(endpoint: str,
                params: dict[str, Any],
                session: Optional[requests.Session] = None) -> int:
    """Fetch total_results for a supported iNaturalist endpoint."""
    if HAS_PYINAT:
        if endpoint == 'observations':
            return int(inat.get_observations(**params).get('total_results', 0))
        if endpoint == 'species_counts':
            return int(inat.get_observation_species_counts(**params).get('total_results', 0))
        if endpoint == 'observers':
            return int(inat.get_observation_observers(**params).get('total_results', 0))
        if endpoint == 'identifiers':
            return int(inat.get_observation_identifiers(**params).get('total_results', 0))
        raise ValueError(f'Unsupported endpoint: {endpoint}')

    endpoint_map = {
        'observations': 'observations',
        'species_counts': 'observations/species_counts',
    ...
    session_to_use = session or requests.Session()
    url = f"https://api.inaturalist.org/v1/{endpoint_map[endpoint]}"
    q = {**params, 'per_page': 0}
    response = session_to_use.get(url, params=q, timeout=40)
    response.raise_for_status()
    return int(response.json().get('total_results', 0))


def run_tricounty_summary(places: Union[dict[int, str], list[int], tuple[int, ...], None] = None,
                          session: Optional[requests.Session] = None) -> pd.DataFrame:
    """Return a compact tri-county summary DataFrame."""
    place_ids = _extract_place_ids(places)
    base_params = {'place_id': place_ids, 'verifiable': True, 'per_page': 0}
    payload = {
        'observations': _inat_total('observations', base_params, session=session),
        'taxa': _inat_total('species_counts', base_params, session=session),
        'identifiers': _inat_total('identifiers', {'place_id': place_ids, 'per_page': 0}, session=session),
        'observers': _inat_total('observers', {'place_id': place_ids, 'per_page': 0}, session=session),
    }
    return pd.DataFrame([payload])


def run_iconic_taxa_counts(places: Union[dict[int, str], list[int], tuple[int, ...], None] = None,
                           throttle_seconds: float = 0.25,
                           session: Optional[requests.Session] = None) -> pd.DataFrame:
    """Return observation counts by iconic taxon for a place scope."""
    place_ids = _extract_place_ids(places)
    if HAS_PYINAT:
        iconic_taxa = list(inat.ICONIC_TAXA.values())
        iconic_taxa = [x for x in iconic_taxa if x != 'Unknown']
    else:
        iconic_taxa = ['Animalia', 'Plantae', 'Fungi', 'Aves', 'Mammalia', 'Reptilia', 'Amphibia', 'Actinopterygii', 'Insecta', 'Arachnida', 'Mollusca']

    rows = []
    for idx, taxon_name in enumerate(iconic_taxa):
        count = _inat_total(
            'observations',
            {'place_id': place_ids, 'iconic_taxa': taxon_name, 'verifiable': True, 'per_page': 0},
            session=session,
        )
        rows.append({'iconic_taxon': taxon_name, 'observations': count})
        if idx < len(iconic_taxa) - 1 and throttle_seconds > 0:
            sleep(float(throttle_seconds))

    return pd.DataFrame(rows).sort_values('observations', ascending=False).reset_index(drop=True)


def _inat_histogram(place_ids: list[int],
                    interval: str,
                    d1: Union[str, dt.date],
                    d2: Union[str, dt.date],
                    session: Optional[requests.Session] = None) -> dict[str, int]:
    if HAS_PYINAT:
        return inat.get_observation_histogram(
            place_id=place_ids,
            interval=interval,
            d1=d1,
            d2=d2,
    ...
    params = {'place_id': place_ids, 'interval': interval, 'd1': d1, 'd2': d2, 'verifiable': 'true'}
    response = session_to_use.get(url, params=params, timeout=40)
    response.raise_for_status()
    return response.json().get('results', {})


def run_observation_histograms(places: Union[dict[int, str], list[int], tuple[int, ...], None] = None,
                               start_year: Union[str, dt.date] = '2008-01-01',
                               start_month: Union[str, dt.date] = '2016-01-01',
                               end_date: Optional[Union[str, dt.date]] = None,
                               session: Optional[requests.Session] = None) -> dict[str, pd.DataFrame]:
    """Return yearly/monthly histogram DataFrames for observations."""
    place_ids = _extract_place_ids(places)
    d2 = end_date or dt.date.today()
    yearly = _inat_histogram(place_ids, interval='year', d1=start_year, d2=d2, session=session)
    monthly = _inat_histogram(place_ids, interval='month', d1=start_month, d2=d2, session=session)

    yearly_df = pd.DataFrame([{'date': k, 'observations': v} for k, v in yearly.items()])
    monthly_df = pd.DataFrame([{'date': k, 'count': v, 'metric': 'Observations'} for k, v in monthly.items()])

    if not yearly_df.empty:
        yearly_df['date'] = pd.to_datetime(yearly_df['date'], errors='coerce')
    if not monthly_df.empty:
        monthly_df['date'] = pd.to_datetime(monthly_df['date'], errors='coerce')
        by_month_num = monthly_df.groupby(monthly_df['date'].dt.month)['count'].sum().to_frame(name='count')
    else:
        by_month_num = pd.DataFrame(columns=['count'])

    return {
        'yearly': yearly_df.sort_values('date'),
        'monthly': monthly_df.sort_values('date'),
        'monthly_by_month_number': by_month_num,
    }


def run_monthly_activity_matrix(places: Union[dict[int, str], list[int], tuple[int, ...], None] = None,
                                session: Optional[requests.Session] = None) -> pd.DataFrame:
    """Return month-of-year counts for observations, taxa, observers, identifiers."""
    place_ids = _extract_place_ids(places)
    observations = []
    taxa = []
    observers = []
    ...
    for month in range(1, 13):
        common = {'place_id': place_ids, 'month': month, 'verifiable': True, 'per_page': 0}
        observations.append(_inat_total('observations', common, session=session))
        taxa.append(_inat_total('species_counts', common, session=session))
        observers.append(_inat_total('observers', common, session=session))
        identifiers.append(_inat_total('identifiers', common, session=session))
...
def run_coming_soon_suite(target_loc: tuple[float, float, float],
                          kinds: Optional[list[str]] = None,
                          norm: Optional[str] = 'overall',
                          limit: int = 7,
                          fetch_images: bool = False,
                          token: Optional[str] = None,
                          session: Optional[requests.Session] = None,
                          use_cache: bool = True) -> dict[str, pd.DataFrame]:
    """Run a batch of coming_soon queries by clade/phenotype."""
    selected_kinds = kinds or ['flowers', 'fruits', 'mushrooms', 'birds', 'herps', 'mammals', 'wugs']
    out = {}
...
def confidence_manimal_accuracy(target_user: str,
                                taxon_query: str,
                                sleep_seconds: float = 0.2,
                                session: Optional[requests.Session] = None) -> dict[str, Any]:
    """Classify identification outcomes for a user within a target taxon scope."""
    from collections import Counter

    base = 'https://api.inaturalist.org/v1'
    session_to_use = session or requests.Session()

    def _get_taxon_id(q: str) -> int:
        r = session_to_use.get(f'{base}/taxa', params={'q': q, 'per_page': 1}, timeout=30)
        r.raise_for_status()
    ...
    def _paged(endpoint: str, params: dict[str, Any]):
        page = 1
        per_page = 200
        while True:
            q = {**params, 'page': page, 'per_page': per_page}
            r = session_to_use.get(f'{base}/{endpoint}', params=q, timeout=60)
            r.raise_for_status()
            payload = r.json()
    ...
    def _taxon_lineage_ids(taxon: Optional[dict[str, Any]]) -> set[int]:
        if not isinstance(taxon, dict):
            return set()
        ids = set(int(x) for x in (taxon.get('ancestor_ids') or []) if x is not None)
        self_id = taxon.get('id')
        if self_id is not None:
            ids.add(int(self_id))
        return ids

    def _classify_id(id_obj: dict[str, Any], obs_obj: Optional[dict[str, Any]]) -> tuple[str, str]:
        if not id_obj.get('current', True):
            return 'withdrawn', 'not current'

        category = str(id_obj.get('category') or '').lower()
        if category in {'leading', 'improving', 'supporting'}:
            return 'validated', f'category={category}'

        id_lineage = _taxon_lineage_ids(id_obj.get('taxon'))
        comm_lineage = _taxon_lineage_ids((obs_obj or {}).get('community_taxon'))

        if id_lineage & comm_lineage:
            return 'validated', 'lineage overlap with community taxon'
        if category == 'maverick':
            return 'overruled', 'category=maverick'
        if comm_lineage:
            return 'overruled', 'no lineage overlap with community taxon'
        return 'unclear', 'no community taxon'

    taxon_id = _get_taxon_id(taxon_query)
    id_list = list(_paged('identifications', {
        'user_id': target_user,
        'taxon_id': taxon_id,
        'current': 'any',
        'order_by': 'created_at',
    ...
    if not id_list:
        empty_df = pd.DataFrame(columns=['classification', 'count', 'percent'])
        return {
    ...
    obs_ids = sorted({(i.get('observation') or {}).get('id') for i in id_list if (i.get('observation') or {}).get('id') is not None})
    obs_map = {}
    for i in range(0, len(obs_ids), 200):
        chunk = obs_ids[i:i + 200]
        r = session_to_use.get(f'{base}/observations', params={'id': ','.join(map(str, chunk)), 'per_page': 200}, timeout=60)
        r.raise_for_status()
        for obs in r.json().get('results', []):
            obs_id = obs.get('id')
            if obs_id is not None:
                obs_map[int(obs_id)] = obs
        if sleep_seconds > 0:
            sleep(float(sleep_seconds))

    rows = []
    for ident in id_list:
        obs_id = (ident.get('observation') or {}).get('id')
        if obs_id is not None:
            obs_id = int(obs_id)
        cls, detail = _classify_id(ident, obs_map.get(obs_id))
        rows.append({
            'identification_id': ident.get('id'),
            'obs_id': obs_id,
            'created_at': ident.get('created_at'),
            'current': ident.get('current'),
            'category': ident.get('category'),
            'maverick': ident.get('maverick'),
            'id_taxon_id': (ident.get('taxon') or {}).get('id'),
            'id_taxon_name': (ident.get('taxon') or {}).get('name'),
    ...
    details_df = pd.DataFrame(rows).sort_values('created_at')
    counts = Counter(details_df['classification'])
    total = len(details_df)
    summary_df = pd.DataFrame([
        {
            'classification': label,
            'count': int(counts.get(label, 0)),
            'percent': (float(counts.get(label, 0)) / float(total) * 100.0) if total else 0.0,
        }
        for label in ['validated', 'overruled', 'withdrawn', 'unclear']
    ])
...
def inaturalismus_notebook(mode: str = 'quick',
                           defaults: Optional[dict[str, Any]] = None,
                           token: Optional[str] = None,
                           session: Optional[requests.Session] = None,
                           render: bool = True) -> dict[str, Any]:
    """Run the multi-workflow inaturalismus pipeline from one helper call.

    Parameters
    ----------
    mode:
        ``'quick'`` for fast baseline outputs, ``'full'`` to include heavier
        iconic-taxa and timeline sections.
    defaults:
        Optional config overrides for place scope, target location, and query
        behavior. See ``DEFAULT_INATURALISMUS_CONFIG`` for supported keys.
    token, session:
        Optional auth/session overrides used by API calls.
    render:
        If True (default), display compact notebook output summaries.

    Returns
    -------
    dict
        A results dictionary keyed by workflow section.
    """
    normalized_mode = str(mode).strip().lower()
    if normalized_mode not in {'quick', 'full'}:
        raise ValueError("mode must be 'quick' or 'full'")

    cfg = dict(DEFAULT_INATURALISMUS_CONFIG)
    if defaults:
        cfg.update(defaults)

    session_to_use = session or get_inat_session(token=token, use_cache=True)
    place_ids = _extract_place_ids(cfg.get('places'))
    ...
    if render:
        _display_md(f"## inaturalismus ({normalized_mode})")
        _display_md('Running workflows...')

    summary_df = run_tricounty_summary(places=cfg.get('places'), session=session_to_use)
    state['results']['tricounty_summary'] = summary_df

    if render:
        _display_md('### Tri-county summary')
        display(summary_df)

    suite = run_coming_soon_suite(
        target_loc=tuple(cfg['target_loc']),
        kinds=list(cfg.get('coming_soon_kinds') or []),
        norm=cfg.get('coming_soon_norm'),
        limit=int(cfg.get('coming_soon_limit', 7)),
        fetch_images=bool(cfg.get('coming_soon_fetch_images', False)),
        token=token,
        session=session_to_use,
        use_cache=True,
    )
    state['results']['coming_soon_suite'] = suite

    if bool(cfg.get('include_lifestages', True)):
        life_stage_suite = run_coming_soon_suite(
            target_loc=tuple(cfg['target_loc']),
            kinds=['caterpillars', 'butterflies'],
            norm='overall',
            limit=min(3, int(cfg.get('coming_soon_limit', 7))),
            fetch_images=bool(cfg.get('coming_soon_fetch_images', False)),
            token=token,
            session=session_to_use,
            use_cache=True,
        )
        state['results']['coming_soon_lifestages'] = life_stage_suite

    if render:
        _display_md('### Coming soon suite (preview rows)')
        for kind_name, frame in suite.items():
            _display_md(f'#### {kind_name}')
            if getattr(frame, 'empty', True):
                print('No results')
                continue
            show_cols = [c for c in ['count', 'taxon.name', 'taxon.preferred_common_name', 'taxon.wikipedia_url'] if c in frame.columns]
            display(frame[show_cols].head(int(cfg.get('coming_soon_limit', 7))))

    if normalized_mode == 'full':
        iconic_df = run_iconic_taxa_counts(
            places=cfg.get('places'),
            throttle_seconds=float(cfg.get('throttle_seconds', 0.25)),
            session=session_to_use,
        )
        histograms = run_observation_histograms(
            places=cfg.get('places'),
            start_year=cfg.get('hist_start_year', '2008-01-01'),
            start_month=cfg.get('hist_start_month', '2016-01-01'),
            end_date=dt.date.today(),
            session=session_to_use,
        )
        monthly_matrix = run_monthly_activity_matrix(
            places=cfg.get('places'),
            session=session_to_use,
        )

        state['results']['iconic_taxa'] = iconic_df
        state['results']['histograms'] = histograms
        state['results']['monthly_activity'] = monthly_matrix

        if render:
            _display_md('### Iconic taxa')
            display(iconic_df)
            _display_md('### Observation histograms')
            display(histograms['yearly'].head(10))
            display(histograms['monthly'].tail(12))
            _display_md('### Month-of-year activity')
            display(monthly_matrix)

    if render:
        _display_md('Done. Results are available in the returned `state` dictionary.')
